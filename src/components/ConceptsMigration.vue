    ...
    <div>    
        ...
        <v-card class="pa-8 mt-2">
            <v-form ref="form" v-model="form_valid" lazy-validation>
                <v-autocomplete dense v-model="from_url" readonly :items="store().concepts" :label="$t('Select a concept')" item-title="name" item-value="url" :rules="RulesSelection(true)"></v-autocomplete>
                <v-autocomplete dense v-model="to" :items="store().concepts" :label="$t('Select a concept to migrate to')" item-title="name" item-value="url" :rules="RulesSelection(true)"></v-autocomplete>
            </v-form>
            <v-card-actions>
                <v-spacer></v-spacer>
