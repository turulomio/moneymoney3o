    ...
    <div>
        <v-data-table  ref="table_ao" :show-select="showselected" v-model="selected" dense :headers="table_headers()" :items="items" class="elevation-1" disable-pagination  hide-default-footer sort-by="datetime" fixed-header v-bind="$attrs" :single-select="false">
            ...
            <template v-slot:[`item.concepts`]="{ item }">
                <div v-html="getObjectPropertyByUrl('concepts', item.concepts, 'name')"></div>
            </template>
        ...
        methods: {
            ...
            table_headers(){
                var r= [
                    ...
                    { title: this.$t('Amount'), key: 'amount', sortable: false, align:'end', width:"8%"},
                    { title: this.$t('Balance'), key: 'balance', sortable: false, align:'end', width:"8%"},
                    { title: this.$t('Comment'), key: 'comment', sortable: true},
                ]
